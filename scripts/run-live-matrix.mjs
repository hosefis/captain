...
const [packageManager, topology] = process.argv.slice(2);
...
const root = process.env.CAPTAIN_MATRIX_ROOT
  ...
  : mkdtempSync(join(tmpdir(), `captain-live-${packageManager}-${topology}-`));
...
console.log(`Live matrix passed: ${packageManager}/${topology}`);
