...
import { createRestBackendClient } from "{adapterNextPackage}";

export const customApiPaymentConfig = {
  processor: "custom-api" as const,
  orchestration: "{orchestration}" as const,
};

export function createCustomApiPaymentAdapter(
  ...
  return {
    async initiateCheckout(ctx: CheckoutContext): Promise<CheckoutResult> {
      ...
      if (response.checkout_url) {
        return { kind: "redirect", url: response.checkout_url };
      }
